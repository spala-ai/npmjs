...
1. If the user supplies an exact project MCP URL, use it directly. Run \`npx --yes ${INSTALLER_PACKAGE_SPEC} --url "<exact-user-url>" --exact-url --client codex --yes --json\`, stop at any reload boundary, then call that project MCP's \`spala_start\`. Do not route the user through public project discovery, replace the URL, infer another project URL, or ask them to authenticate to a different Spala host.
2. Otherwise, inspect \`.spala/project.json\` first when it exists. If it is valid and its named project MCP is configured, use that project MCP directly and call its \`spala_start\`; do not authenticate or route through the public MCP. A public-MCP authentication failure does not block work when the bound project MCP succeeds. If the binding is valid but its named MCP is missing, reinstall that exact binding with \`npx --yes ${INSTALLER_PACKAGE_SPEC} --url "<binding.mcpUrl>" --exact-url --name "<binding.serverName>" --client codex --yes --json\`, then stop at the reload boundary. Only when no valid project binding exists, run \`codex mcp get spala_public_mcp\` as the public configuration check. If it succeeds, do not run \`init\` or \`login\`; continue directly to its \`spala_start\`. If and only if it reports that the server does not exist, run exactly \`npx --yes ${INSTALLER_PACKAGE_SPEC} init --client codex --yes --json\`, then stop at the returned reload boundary. Do not use web search or docs lookup as a substitute for this check.
